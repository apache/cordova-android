...
function recursivelyFindFiles (dir, extension) {
    ...
    if (fs.existsSync(dir)) {
        ...
        for (let i = 0; i < results.length; i++) {
            let item = path.resolve(dir, results[i]);
            let stat = fs.statSync(item);
            if (stat.isDirectory()) {
                apks = apks.concat(recursivelyFindFiles(item, extension));
            } else {
                if (path.parse(item).ext === '.' + extension) {
                    apks.push(item);
                }
            }
        }
    ...
    prepEnv (opts) {
        ...
        return check_reqs.check_gradle()
            .then(function (gradlePath) {
                ...
                fs.chmodSync(gradleWrapperPropertiesPath, 0o644);

                utils.replaceFileContents(gradleWrapperPropertiesPath, distributionUrlRegex, 'distributionUrl=' + distributionUrl);
                ...
                if (opts.packageInfo) {
                    ...
                    fs.removeSync(propertiesFilePath);
                }
            });
    }
    ...
    clean (opts) {
        ...
        return execa(wrapper, args, { stdio: 'inherit' })
            .then(function () {
                ...
                fs.removeSync(path.join(builder.root, 'out'));
