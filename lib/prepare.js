...
function updateProjectSplashScreen (platformConfig, locations) {
    ...
    [
        ...
        case 'windowSplashScreenBackground':
            ...
            themeTargetNode.text = '@colors/cdv_splashscreen_background';
            break;
        ...
        case 'windowSplashScreenIconBackgroundColor':
            ...
            // force the themes value to `@colors/cdv_splashscreen_icon_background`
            if (!cdvConfigPrefValue && themeTargetNode) {
                ...
                themeTargetNode.text = '@colors/cdv_splashscreen_icon_background';
            }
            break;
...
function cleanupAndSetProjectSplashScreenImage (srcFile, destFilePath, possiblePreviousDestFilePath, cleanupOnly = false) {
    ...
    if (!cleanupOnly && fs.existsSync(srcFile)) {
...
function updateProjectSplashScreenImage (locations, themeKey, cdvConfigPrefKey, cdvConfigPrefValue = '') {
    ...
    const defaultSrcFilePath = require.resolve('cordova-android/templates/project/res/drawable/' + destFileNameExt);

    if (!cdvConfigPrefValue || !fs.existsSync(cdvConfigPrefValue)) {
        events.emit('verbose', `The "${cdvConfigPrefKey}" is undefined or the file path does not exist. Cordova's default will be used.`);
