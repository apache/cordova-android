...
function updateProjectSplashScreen (platformConfig, locations) {
    ...
    const splashscreenData = [
        ...
        key => {
            const prefKey = 'Android' + key.charAt(0).toUpperCase() + key.slice(1);
            let value = platformConfig.getPreference(prefKey, this.platform);

            if (key === 'windowSplashScreenBackground') {
                // use the user defined value for "colors.xml"
                updateProjectSplashScreenBackgroundColor(value, locations);
                // force the themes value to `@colors/cdv_splashscreen_background`
                value = '@colors/cdv_splashscreen_background';
            } else if (key === 'windowSplashScreenAnimatedIcon') {
                ...
                //  - Copy png in correct mipmap dir with name "ic_cdv_splashscreen.pn"
                // If "xml":
    ...
    splashscreenData.forEach(obj => {
        splashScreenTheme.find(`item[@name="${obj.key}"]`).text = obj.value.replace(/'/g, '\\\'');
    });
